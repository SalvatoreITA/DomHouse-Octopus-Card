      ...
      <div class="card-config">
        <div class="vertical-inputs">
            <ha-selector
                .hass=${this.hass}
                .selector=${{ text: {} }}
                .value=${this._config.title !== undefined ? this._config.title : '⚖️ Confronto vs Sito Ufficiale'}
                .configValue=${"title"}
...
class DomHouseMyContractCard extends LitElement {
  ...
  // --- HELPER FIX PER GLI STATI PENDING (ISSUE #1) ---
  _getStatoFornitura(rawStato) {
    if (!rawStato) return { testo: 'N/A', colore: '#888' };
    const s = rawStato.toUpperCase();
    if (s === 'ON_SUPPLY' || s.includes('ATTIV'))
      return { testo: 'FORNITURA ATTIVA', colore: '#00e676' };
    if (s.includes('PENDING') || s.includes('SWITCH_IN'))
      return { testo: 'CAMBIO IN CORSO', colore: '#ffd600' };
    return { testo: 'FORNITURA CESSATA', colore: '#ff5252' };
  }
  ...
  render() {
    ...
    if (hasLuce) {
      ...
      const l_raw_stato = this.getStringValue(this._config.l_stato);
      ...
      const l_stato = this._getStatoFornitura(l_raw_stato);
      const l_textStato = l_stato.testo;
      const l_statoColor = l_stato.colore;
    ...
    if (hasGas) {
      ...
      const g_raw_stato = this.getStringValue(this._config.g_stato);
      ...
      const g_stato = this._getStatoFornitura(g_raw_stato);
      const g_textStato = g_stato.testo;
      const g_statoColor = g_stato.colore;
      ...
      <div class="card-config">
        <div class="vertical-inputs">
            <ha-selector
                .hass=${this.hass}
                .selector=${{ text: {} }}
                .value=${this._config.title !== undefined ? this._config.title : '🐙 Octopus: Il Mio Contratto'}
                .configValue=${"title"}
                .label=${"Titolo Card (lascia vuoto per nasconderlo)"}
                @value-changed=${this._valueChanged}>
            </ha-selector>
